...
import torchvision
from pytorch_image_classification.models.imagenet.resnet_features import Res_mlp
...
def train(epoch, config, model, optimizer, scheduler, loss_func, train_loader,
    ...
    for step, (data, targets) in enumerate(train_loader):
        ...
        if get_rank() == 0 and step == 1 and global_step == 1:
...
def validate(epoch, config, model, loss_func, val_loader, logger,
    ...
    with torch.no_grad():
        ...
        for step, (data, targets) in enumerate(val_loader):
            ...
            y_p = targets.tolist()
            y_bar_p = torch.max(outputs, 1)[1].tolist()

            y.extend(y_p)
            y_bar.extend(y_bar_p)

            # print('=======output start=======', torch.topk(outputs, 1)[1].tolist(), "=====output done")
        ...
        print('=======target start=======', y, "=====target done")
        print('=======outputs start=======', y_bar, "=====outputs done")
        ...
        conf_mat = confusion_matrix(y, y_bar)
        print(conf_mat)
        # if acc1_meter.avg >=0.80:
        #     df_mat = pd.DataFrame(conf_mat)
        #     plt.figure(figsize=(12, 10))
        #     sn.set(font_scale=1)  # for label size
        #     ax = sn.heatmap(df_mat, annot=True, annot_kws={"size": 12}, cmap="Blues", fmt='g')  # font size
        ...
        # print(confusion_matrix(y, y_bar))
        print('===================')
    ...
    return acc1_meter.avg, conf_mat, y, y_bar, val_loader.dataset.samples, val_loader.dataset.class_to_idx
...
def main():
    ...
    print("SIZE OF TRAIN LOADER")
    print(len(train_loader.dataset))

    base_model = create_model(config)
    model = Res_mlp(base_model,mode=0,act='Relu').to(config.device)
    ...
    for epoch, seed in enumerate(epoch_seeds[start_epoch:], start_epoch):
        ...
        if config.train.val_period > 0 and (epoch % config.train.val_period
                                            == 0):
            accval, conf_mat, y, y_bar, data, class_dict = validate(epoch, config, model, val_loss, val_loader, logger,
                              tensorboard_writer)
        ...
        ii=0 
        for lab in range(0, len(y)):

            if y[lab] != y_bar[lab]:
                bad_images.append(data[ii])
                gt_list.append(y[ii])
                pred_list.append(y_bar[ii])
            ii=ii+1
        # df_bad_images = pd.DataFrame(bad_images,columns=['Bad Images'])
        # df_bad_images.to_excel(str(output_dir)+'/'+str(epoch)+'_bad_images.xlsx')


        if (epoch == 0) or (epoch == 1) or (epoch == config.scheduler.epochs) or accval >= 0.99:
            checkpoint_config = {
                'epoch': epoch,
                'global_step': global_step,
                'config': config.as_dict(),
            ...
            t -= 0.5  # Subtract 0.5 from the top
            plt.ylim(b, t)  # update the ylim(bottom, top) values
            plt.savefig(str(output_dir)+'/'+'conf_mat_'+str(epoch)+'.jpg')

            # dd = {'Bad Images': bad_images[0], 'Label': bad_images[1], 'Ground Truth': y, 'Predicted': y_bar}

            df_bad_images = pd.DataFrame()
            df_bad_images['Bad Images'] = pd.Series(bad_images)
            # df_bad_images['Label'] = pd.Series(bad_images[1])
            df_bad_images['Ground Truth'] = pd.Series(gt_list)
            df_bad_images['Predicted'] = pd.Series(pred_list)
            df_bad_images['Class Identifier'] = pd.Series(str(class_dict))
            # df_bad_images = pd.DataFrame(bad_images, y, y_bar, columns=['Bad Images','Label', 'Ground Truth', 'Predicted'])
            df_bad_images.to_excel(str(output_dir) + '/' + str(epoch) + '_bad_images.xlsx')
