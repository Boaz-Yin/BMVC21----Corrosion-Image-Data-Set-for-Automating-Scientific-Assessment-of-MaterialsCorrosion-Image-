...
def evaluate(config, model, test_loader, loss_func, logger):
    ...
    with torch.no_grad():
        for data, targets in tqdm.tqdm(test_loader):
            ...
            grtruth.append(targets.cpu().numpy())
    ...
    print(grtruth)
    print(labels)
    conf_mat = confusion_matrix(labels, grtruth)
    print(confusion_matrix(labels, grtruth))
    print(data)
    #print(data.__dict__)

    return preds, probs, labels, loss_meter.avg, accuracy, conf_mat, grtruth, data, test_loader
...
def main():
    ...
    preds, probs, labels, loss, acc, conf_mat, grtruth, data, test_loader = evaluate(config, model, test_loader,
                                               test_loss, logger)

    print(test_loader.dataset.samples)
    names = test_loader.dataset.samples
