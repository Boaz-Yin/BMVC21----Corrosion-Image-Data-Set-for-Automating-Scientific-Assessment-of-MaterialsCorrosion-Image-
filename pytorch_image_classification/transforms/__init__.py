...
def create_imagenet_transform(config: yacs.config.CfgNode,
    ...
    if is_train:
        ...
        if config.augmentation.use_gaussianblur:
            transforms.append(GaussianBlur(config))
        
        if config.augmentation.use_randomresizecrop:
            transforms.append(RandomResizeCrop(config))

        if config.augmentation.use_random_vertical_flip:
            transforms.append(RandomVerticalFlip(config))

        if config.augmentation.use_random_perspective:
            transforms.append(RandomPerspective(config))

        if config.augmentation.use_random_rotation:
            transforms.append(RandomRotation(config))
            transforms.append(Resize(config))
        ...
        if config.augmentation.use_center_crop:
            transforms.append(CenterCrop(config))
